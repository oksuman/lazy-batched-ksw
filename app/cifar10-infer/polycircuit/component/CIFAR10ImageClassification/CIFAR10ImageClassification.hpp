...
public:
    ...
    Ciphertext evaluate_lazy() override
    {
        const std::vector<std::vector<double>> weightsMatrix
        {
            #include "CIFAR10ImageClassification_weights.txt"
        ...
        tmp = m_cc->EvalMult(m_inputC, m_cc->MakeCKKSPackedPlaintext(weightsMatrix.at(4)));
        tmp = m_cc->EvalLazyRotate(tmp, -1);
        enc_out = m_cc->EvalLazyAdd(tmp, enc_out);

        tmp = m_cc->EvalMult(m_inputC, m_cc->MakeCKKSPackedPlaintext(weightsMatrix.at(5)));
        tmp = m_cc->EvalLazyRotate(tmp, -2);
        enc_out = m_cc->EvalLazyAdd(tmp, enc_out);

        tmp = m_cc->EvalMult(m_inputC, m_cc->MakeCKKSPackedPlaintext(weightsMatrix.at(6)));
        tmp = m_cc->EvalLazyRotate(tmp, -3);
        enc_out = m_cc->EvalLazyAdd(tmp, enc_out);
        ...
        // Print key dependencies before EvalBatchedKS
        // std::cout << "[EvalBatchedKS #1] Key dependencies before batched key switch: [ ";
        // for (const auto& keyIdx : enc_out->GetElementKeyIndexVector()) {
        //     std::cout << keyIdx << " ";
        // }
        // std::cout << "]" << std::endl;

        enc_out = m_cc->EvalBatchedKS(enc_out);

        tmp = m_cc->EvalDirectRotate(enc_out, 1600);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        tmp = m_cc->EvalDirectRotate(enc_out, 800);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        tmp = m_cc->EvalDirectRotate(enc_out, 400);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        tmp = m_cc->EvalDirectRotate(enc_out, 200);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        tmp = m_cc->EvalDirectRotate(enc_out, 100);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        tmp = m_cc->EvalDirectRotate(enc_out, 50);
        enc_out = m_cc->EvalAdd(enc_out, tmp);

        lbcrypto::Ciphertext<ElementType> enc_out1 = m_cc->EvalDirectRotate(enc_out, 40);
        tmp = m_cc->EvalDirectRotate(enc_out, 20);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        tmp = m_cc->EvalDirectRotate(enc_out, 10);
        enc_out = m_cc->EvalAdd(enc_out, tmp);
        enc_out = m_cc->EvalAdd(enc_out, enc_out1);
